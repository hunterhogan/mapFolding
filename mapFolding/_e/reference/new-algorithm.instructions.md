...
| `_beDRY.py` | Generic helpers: `ChoicesLeaf`, dimension products, and grouping helpers. |
| `_disaggregation.py` | `ChoicesLeaf` → iterator of leaves. |
| `theTypes.py` | `_e` type aliases such as `Leaf`, `ChoicesLeaf`, and `PermutationSpace`. |
...
| `_2上nDimensionalChoicesLeaf.py` | Per-pile `ChoicesLeaf` builders. |
...
- `ChoicesLeaf` is a `gmpy2.mpz` bitset with a sentinel bit.
