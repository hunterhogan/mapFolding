...
@numba.jit(parallel=False, _nrt=True, boundscheck=False, error_model='numpy', fastmath=True, forceinline=True, looplift=False, no_cfunc_wrapper=True, no_cpython_wrapper=True, nogil=True, nopython=True)
def doWhile(connectionGraph: NDArray[integer[Any]], foldsTotal: NDArray[integer[Any]], my: NDArray[integer[Any]], potentialGaps: NDArray[integer[Any]], the: NDArray[integer[Any]], track: NDArray[integer[Any]], Z0Z_initializeUnconstrainedLeaf: Optional[bool] ):
    while my[indexMy.leaf1ndex.value] > 0:
        ...
        if initializationConditionUnconstrainedLeaf(my, Z0Z_initializeUnconstrainedLeaf):
            break
    return foldsTotal, my, potentialGaps, track
...
def countFoldsCompileBranch(connectionGraph: NDArray[integer[Any]], foldsTotal: NDArray[integer[Any]],
                            my: NDArray[integer[Any]], potentialGaps: NDArray[integer[Any]], the: NDArray[integer[Any]], track: NDArray[integer[Any]],
                            obviousFlagForNumba: bool):
    if obviousFlagForNumba:
        foldsTotal, _1, _2, _3 = doWhile(connectionGraph, foldsTotal, my, potentialGaps, the, track, Z0Z_initializeUnconstrainedLeaf=False)
    else:
        foldsTotal = doTaskIndices(connectionGraph, foldsTotal, my, potentialGaps, the, track)
...
def countFoldsCompiled(connectionGraph: NDArray[integer[Any]], foldsTotal: NDArray[integer[Any]], my: NDArray[integer[Any]], potentialGaps: NDArray[integer[Any]], the: NDArray[integer[Any]], track: NDArray[integer[Any]]) -> int:
    ...
    _0, my, potentialGaps, track = doWhile(connectionGraph, foldsTotal, my, potentialGaps, the, track, Z0Z_initializeUnconstrainedLeaf=True)

    obviousFlagForNumba = the[indexThe.taskDivisions.value] == int(False)

    foldsTotal = countFoldsCompileBranch(connectionGraph, foldsTotal, my, potentialGaps, the, track, obviousFlagForNumba)
