...
def getValuesKnown(oeisID: OEISid) -> dict[int, int]:
	...
	"""
	return dictionaryOEIS.get(oeisID, dict[str, dict[int, int]]()).get('valuesKnown', {-1: -1})

#================== Meat grinders =================================================================

def _getMetadata(pathFilenameCache: Path, url: str) -> str | None:
...
def _getMetadata_bFile(oeisID: OEISid) -> dict[int, int]:
	...
	oeisData: str | None = _getMetadata(pathFilenameCache, url)
...
def _getMetadataAFile(oeisID: OEISid) -> tuple[str, int]:
	...
	oeisInformation: str | None = _getMetadata(pathFilenameCache, url)

	if not oeisInformation:
		return "Not found", errorL33T
