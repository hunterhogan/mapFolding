...
def makeJob(listDimensions: List[int], pathJobs: Union[str, os.PathLike[Any]] = "jobs"):
    ...
    dtypeDefault: Final = numpy.uint16
    ...
    for taskIndex in list(range(leavesTotal)):
        ...
        pathFilenameJobWriteStream = pathlib.Path(pathJobWriteStream, f"{str(taskIndex).zfill(zeroPadding)}.pkl").write_bytes(pickle.dumps(dictionaryState))
