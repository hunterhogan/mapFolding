"""
Goal: "foldings" algorithm implemented in JAX

Plan:
    - Start from a numba implementation of the "foldings" algorithm
        - The implementation has been thoroughly tested
        - It is supported by multiple pytest tests
        - It is part of a package that handles many other things
    - Convert code to JAX in obscenely small increments
    - Test every change using the existing pytest tests
    - Add highly targeted tests for tiny slices of the JAX implementation
    - Try to use the "most correct" JAX features and syntax
    - Use the JAX typing module to ensure correctness
    - Use other tools to ensure correctness

Notes:
    - Use "babbage.py" to create the data structures while they are still passed as numpy
    - When "pider.py" is ready to receive JAX data structures, use "stoo.py" to create the data structures and call "pider.py"
"""
import numpy
import jax
import jaxtyping

# Indices of array `THE`/`the`. Static integer values
...
def countFoldings(track: numpy.ndarray[numpy.int32, numpy.dtype[numpy.int32]], 
                    potentialGaps: numpy.ndarray[numpy.int32, numpy.dtype[numpy.int32]],
                    THE: numpy.ndarray[numpy.int32, numpy.dtype[numpy.int32]], 
                    connectionGraph: numpy.ndarray[numpy.int32, numpy.dtype[numpy.int32]]):
    the = jax.numpy.asarray(THE, dtype=jax.numpy.int32)
    ...
    while activeLeaf1ndex > 0:
        if activeLeaf1ndex <= 1 or track[leafBelow][0] == 1:
            if activeLeaf1ndex > int(the[leavesTotal]):
                ...
                # Track possible gaps 
                gap1ndexLowerBound = jax.numpy.int8(track[gapRangeStart][activeLeaf1ndex - 1])
                # Reset gap index
                ...
                for dimension1ndex in range(1, int(the[dimensionsTotal]) + 1):
                    if connectionGraph[dimension1ndex][activeLeaf1ndex][activeLeaf1ndex] == activeLeaf1ndex:
                        ...
                        while leaf1ndexConnectee != activeLeaf1ndex:
                            if int(the[taskDivisions]) == 0 or activeLeaf1ndex != int(the[taskDivisions]) or leaf1ndexConnectee % int(the[taskDivisions]) == int(the[taskIndex]):
